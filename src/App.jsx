  ...
  const ttData=useMemo(()=>{
    ...
    members.forEach(m=>{
      m.trials.filter(t=>t.event_no).forEach(t=>{
        ...
        map[no].trials.push({...t,memberName:m.name,memberCategory:t.category||m.category,memberId:m.id,memberOfficial:t.official!==false});
    ...
    members.forEach(m=>{
      ...
      m.trials.forEach(t=>{
        ...
        if(dist.includes(q)||evt.includes(q)||catL.includes(q)||catS.includes(q)){
          trialHits.push({...t,memberName:m.name,memberId:m.id,memberOfficial:t.official!==false});
        }
      });
    });
